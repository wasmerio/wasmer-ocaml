(** This module contains all bindings. Currently, only bindings to functions in
    wasm.h have been implemented. *)
...
(** A pointer was accessed but its ownership realtive to the caller was not high
    enough. For example, trying to get a pointer using get_ptr while the pointer
    was given away will raise this exception. *)
...
type object_state =
  ...
  | State_Dependent of
      ...
      (** The object's state depends on another. The first function is to get
          the current state, the second function updates it. *)
  | State_PassedAway  (** The current pointer has been given away. *)

val get_real_state : object_state -> object_state
(** Gets the real state of the object_state. Never returns
...
(** Gets the real state of the object_state, and reduces the ownership to
    read-only if required. This is the same as [get_real_state st] if the result
    is not {!const:WasmerBindings.object_state.State_Owned} or
...
module type VectorType = sig
  ...
  val to_dependent :
    ...
    (unit -> object_state) * object_state dependent_update_func ->
    owning_struct
  (** Transform an element of the vector back to a pointer, which ownership is
...
(** Ownable objects are the basic structure that maintains an ownership state on
    C objects. *)
module OwnableObject (O : ObjectType) : sig
  ...
  (** Make a new owning structure from a pointer. See also
      {!val:WasmerBindings.OwnableObject.make_from_unsafe}.

      @raise Returned_null If the pointer is NULL *)

  val make_from_unsafe : O.t ptr -> object_state -> s
  (** Make a new owning structure from a pointer. Does not raise an exception if
      NULL is given. See also
      {!val:WasmerBindings.OwnableObject.make_from_raise}. *)

  val make_from_raise_data : O.t ptr -> object_state -> O.d -> s
  (** Make a new owning structure from a pointer, with an additional data. See
      also {!val:WasmerBindings.OwnableObject.make_from_unsafe_data}.

      @raise Returned_null If the pointer is NULL *)

  val make_from_unsafe_data : O.t ptr -> object_state -> O.d -> s
  (** Make a new owning structure from a pointer, with an additional data. Does
      not raise an exception if NULL is given. See also
      {!val:WasmerBindings.OwnableObject.make_from_raise_data}. *)
  ...
  (** Sets the current ownership state to
      {!const:WasmerBindings.object_state.State_PassedAway}. Ignores the current
      ownership. *)

  val grab_ownership : s -> O.t ptr
  (** [grab_ownership self] gives the ownership of the pointed object to the
      caller code. It is equivalent to
      [let p = get_ptr self in lose_ownership self; p], but with additional
      checks for the current ownership.
  ...
  (** Deletes the object if required, then sets the ownership to
      ...
      This is only to be used in own-out function arguments in the C API or
      equivalent.
  ...
  (** Returns the underlying pointer, requiring a sufficient ownership state.
      ...
      @raise Invalid_access
        If the current state is lower than
  ...
  val delete : s -> unit
  (** Frees the underlying pointer. A side effect is to mark the object as given
      away. *)
end
...
module DeclareOwn (T : StructType) : sig
  ...
  type t
  (** The C unique (abstract) base type for the structure. the structure type is
      therefore [t structure]. *)
  ...
  module O : ObjectType with type t = t structure with type d = T.d

  (**/**)

  (** This structure maintains an ownership status. See also
      {!module:WasmerBindings.OwnableObject}. *)
...
module Byte : sig
  ...
  module V :
    VectorType with type data_type = byte with type owning_struct = byte
...
module Val : sig
  ...
  module V :
    VectorType with type data_type = t structure with type owning_struct = s
...
module DeclareShareableRef (T : StructType) : sig
  ...
  module Shared : module type of struct
    include DeclareOwn (S)
  end
...
module Frame : sig
  ...
  module V :
    VectorType with type data_type = t structure ptr with type owning_struct = s
...
module Module : sig
  include module type of struct
    include DeclareRefBase (Module_T)
  end
...
module Extern : sig
  ...
  module Vec : sig
    ...
    val split_kind :
      s -> Func.s list * Global.s list * Table.s list * Memory.s list

    val split_kind_const :
      s -> Func.s list * Global.s list * Table.s list * Memory.s list
  end
